...
\subsubsection{Co je časová řada}
...
\subsubsection{Komponenty časové řady}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.76\linewidth]{assets/images/casove-rady-panelova-data/time-series-decomposition.png}
  \caption[Rozklad časové řady]{Aditivní rozklad časové řady na pozorovanou řadu, trend, sezónní složku
  a rezidua. Popis obrázku: horní panel ukazuje původní řadu, další panely oddělují hladký trend,
  opakující se sezónní vzorec a zbytkovou náhodnou složku. Takový rozklad pomáhá rozhodnout, zda má
  smysl modelovat trend, sezónnost nebo autokorelaci. Zdroj obrázku: Junkie.dolphin,
  \href{https://commons.wikimedia.org/wiki/File:Group.size.1\_decompose.png}{Wikimedia Commons},
...
\subsubsection{Autokorelace, ACF a PACF}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.86\linewidth]{assets/images/casove-rady-panelova-data/acf-pacf.png}
  \caption[ACF a PACF]{Srovnání ACF a PACF pro autoregresní proces. Popis obrázku: vlevo ACF postupně
  doznívá, zatímco vpravo PACF ukazuje několik výrazných zpoždění a další hodnoty už leží v pásmu šumu.
  Takové chování pomáhá při orientační volbě řádu AR složky, ale v praxi se musí doplnit diagnostikou
  reziduí a informačními kritérii. Zdroj obrázku: Moon motif,
  \href{https://commons.wikimedia.org/wiki/File:Autocorrelation\_Function\_vs.\_Partial\_Autocorrelation\_Function.png}{Wikimedia Commons},
  licence CC BY-SA 4.0.}
  \label{fig:acf-pacf}
\end{figure}

\subsubsection{Základní lineární modely časových řad}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\linewidth]{assets/images/casove-rady-panelova-data/random-walk.png}
  \caption[Random walk]{Osm simulovaných náhodných procházek. Popis obrázku: každá trajektorie začíná
  blízko stejné hodnoty, ale šoky se kumulují, takže řady se mohou dlouhodobě vzdalovat bez návratu ke
  stabilní střední hodnotě. To vystihuje intuici jednotkového kořene a proč je u takových řad často nutné
  diferencování. Zdroj obrázku: Morn,
  \href{https://commons.wikimedia.org/wiki/File:Random\_Walk\_example.svg}{Wikimedia Commons},
  licence GFDL 1.2 nebo novější.}
  \label{fig:random-walk}
\end{figure}

\paragraph{Ceny akcií a lineární regrese.}
U cen akcií je potřeba být opatrný. Samotná cena často bývá nestacionární a může se chovat podobně jako
random walk, takže regrese cen v úrovních snadno vytvoří zdánlivý vztah a vysoké \(R^2\). V praxi se
častěji modelují výnosy nebo log-výnosy:
\[
  r_t=\Delta \log(P_t),
\]
testuje se stacionarita, autokorelace a volatilita. Lineární regrese může být užitečná pro jednoduchý
benchmark nebo vysvětlení vztahu k faktorům, ale predikce cen akcií je obtížná kvůli šumu, změnám režimu,
efektivnosti trhu a riziku overfittingu.
...
\subsubsection{Jednotkový kořen a ADF test}
...
\subsubsection{Regrese s časovými řadami a autokorelace chyb}
...
\subsubsection{Regresní modely se zpožděnými proměnnými}
...
\subsubsection{Predikce a volba modelu}
...
\subsubsection{Čas do události a odhad doby do prolomení}
Některé otázky nejsou klasická predikce hodnoty \(y_t\), ale predikce času do události: kdy zákazník
odejde, kdy zařízení selže nebo kdy se podaří prolomit algoritmus. Pokud pozorujeme délku čekání do
události, je vhodné uvažovat \term{survival analýzu}.

Základní pojmy:
\begin{itemize}
    \item \term{survival funkce} \(S(t)=P(T>t)\) říká pravděpodobnost, že událost nenastane do času \(t\);
    \item \term{hazard} popisuje okamžité riziko události v čase \(t\), podmíněně tím, že do té doby
    nenastala;
    \item \term{cenzorování} znamená, že u části jednotek událost během pozorování nenastala, takže
    známe jen dolní mez času do události.
\end{itemize}

U odhadu času prolomení algoritmu je potřeba zohlednit velikost instance, parametry algoritmu, hardware,
počet pokusů, případné cenzorování a nejistotu. Pokud pozorujeme opakované počty pokusů v čase, mohou
být vhodné i modely počtů nebo časové řady; pokud pozorujeme přímo dobu do úspěchu, dává větší smysl
survival model nebo regrese času do události.

\subsection{Vícerozměrné časové řady, VAR a kointegrace}

\subsubsection{Vícerozměrné časové řady}
...
\subsubsection{Grangerova kauzalita}
...
\subsubsection{Integrace a kointegrace}
...
\subsubsection{Co jsou panelová data}
...
\subsubsection{Základní panelový model}
...
\subsubsection{Random Effects estimator}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.76\linewidth]{assets/images/casove-rady-panelova-data/fe-re.png}
  \caption[FE a RE při korelované heterogenitě]{Simulační srovnání fixed effects a random effects
  odhadu v situaci, kdy je vhodný FE přístup. Popis obrázku: hustota FE odhadů je centrovaná blízko
  skutečné hodnoty parametru, zatímco RE odhady jsou posunuté; obrázek ilustruje, proč je předpoklad
  nekorelovanosti $c_i$ s regresory pro RE klíčový. Zdroj obrázku: SEM,
  \href{https://commons.wikimedia.org/wiki/File:FE-RE.png}{Wikimedia Commons},
  licence CC BY 3.0.}
  \label{fig:fe-re}
\end{figure}
...
\subsubsection{Časové efekty a two-way fixed effects}
...
\subsubsection{Robustní inference v panelech}
...
\subsubsection{Dynamické panelové modely}
...
\subsection{Shrnutí rozdílů mezi hlavními modely}
...
\subsection{Nejdůležitější rovnice k zapamatování}
...
U časových řad je první otázka vždy stacionarita. Pokud jsou dvě řady trendové nebo $I(1)$, obyčejná
regrese v úrovních může být zdánlivá. Nejdřív je potřeba řešit trend, jednotkový kořen a případně
kointegraci. Diferencování ale není automaticky nejlepší řešení: pokud jsou řady kointegrované, samotné
diference zahodí dlouhodobý vztah a vhodnější je ECM nebo VECM.

Grangerova kauzalita není kauzalita v silném ekonomickém smyslu. Znamená, že minulost jedné proměnné
zlepšuje predikci druhé proměnné po kontrole její vlastní minulosti. Pro kauzální interpretaci je pořád
potřeba věcný identifikační argument.

U panelových dat je klíčový předpoklad o nepozorované heterogenitě $c_i$. FE dovoluje korelaci mezi
$c_i$ a regresory, ale neumí odhadnout efekty časově neměnných proměnných. RE je efektivnější a umí
časově neměnné proměnné, ale jen pokud $c_i$ není korelované s regresory. Hausmanův test je pomůcka,
ne náhrada věcného úsudku.

V panelech se standardní chyby často musí clusterovat, typicky podle jednotek. U dynamických panelů je
navíc potřeba myslet na Nickell bias a kontrolovat Arellano--Bond diagnostiky, hlavně absenci AR(2) v
diferencovaných reziduích a přiměřenost instrumentů.

\subsection{Shrnutí ke zkoušce}

Časové řady řeší závislost pozorování v čase. Základní postup je řadu vykreslit, zkontrolovat trend,
sezónnost, stacionaritu a autokorelaci, potom volit mezi AR, MA, ARMA, ARIMA, SARIMA, ADL/ARDL,
VAR nebo VECM podle povahy dat a cíle analýzy. Pro predikci nestačí dobrý fit v trénovacích datech;
důležitá je out-of-sample chyba a kontrola reziduí.

Panelová data kombinují časovou a průřezovou dimenzi. Jejich hlavní výhoda je kontrola časově neměnné
nepozorované heterogenity. Pooled OLS je jednoduchý, ale často riskuje omitted variable bias. FD a FE
odstraňují $c_i$, RE stojí na silnějším předpokladu nekorelovanosti $c_i$ s regresory, CRE/Mundlak
pomáhá tento předpoklad uvolnit a two-way FE přidává společné časové šoky.

\subsection{Zdroje}
\begin{itemize}
    \item statsmodels User Guide:
    \href{https://www.statsmodels.org/stable/tsa.html}{Time Series analysis}.
    \item statsmodels User Guide:
    \href{https://www.statsmodels.org/stable/vector\_ar.html}{Vector Autoregressions}.
    \item linearmodels Documentation:
    \href{https://bashtage.github.io/linearmodels/panel/examples/examples.html}{Panel Data Model Estimation}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:Group.size.1\_decompose.png}{File:Group.size.1\_decompose.png}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:Autocorrelation\_Function\_vs.\_Partial\_Autocorrelation\_Function.png}{ACF/PACF plot}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:Random\_Walk\_example.svg}{File:Random\_Walk\_example.svg}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:FE-RE.png}{File:FE-RE.png}.
\end{itemize}
