...
\begin{aligned}
&\text{reálný problém}
\rightarrow \text{rozhodovací proměnné}
\rightarrow \text{účelová funkce} \\
&\rightarrow \text{omezení}
\rightarrow \text{výpočet a interpretace řešení}.
\end{aligned}
\]
...
\[
\begin{gathered}
\text{Pokud existuje optimální řešení LP,}\\
\text{existuje také v některém vrcholu přípustné množiny.}
\end{gathered}
\]
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.56\linewidth]{assets/images/optimalizacni-modely/lp-feasible-region.png}
  \caption[Přípustná oblast LP]{Geometrická interpretace lineárního programu. Popis obrázku: šedá oblast je množina řešení splňujících všechna lineární omezení; optimum lineární účelové funkce se hledá na hranici a při jednoznačném optimu v některém vrcholu. Zdroj: \href{https://commons.wikimedia.org/wiki/File:Linear\_Programming\_Feasible\_Region.svg}{Wikimedia Commons}, autor Inductiveload, public domain.}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.72\linewidth]{assets/images/optimalizacni-modely/branch-and-bound.png}
  \caption[Branch-and-bound strom]{Schematický strom branch-and-bound. Popis obrázku: původní problém se větví na podproblémy; větve se postupně vyhodnocují pomocí mezí a neperspektivní nebo nepřípustné uzly se ořezávají. Zdroj: \href{https://commons.wikimedia.org/wiki/File:Branch\_and\_Bound.png}{Wikimedia Commons}, autor Enrico Strocchi, licence CC BY-SA 3.0.}
\end{figure}
...
\begin{aligned}
&\text{velká LP oblast}
\rightarrow \text{přidání řezů}
\rightarrow \text{menší LP oblast}\\
&\rightarrow \text{vrcholy blíže celočíselným řešením}.
\end{aligned}
\]
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.58\linewidth]{assets/images/optimalizacni-modely/tsp-solution.png}
  \caption[Řešení TSP]{Příklad řešení problému obchodního cestujícího. Popis obrázku: body představují města a čára ukazuje uzavřenou trasu, která každé město navštíví právě jednou a vrací se do výchozího bodu. Zdroj: \href{https://commons.wikimedia.org/wiki/File:GLPK\_solution\_of\_a\_travelling\_salesman\_problem.svg}{Wikimedia Commons}, autor Xypron, public domain.}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\linewidth]{assets/images/optimalizacni-modely/aco-tsp.png}
  \caption[Ant colony optimization pro TSP]{Princip ant colony optimization na TSP. Popis obrázku: umělí mravenci opakovaně konstruují trasy, dobré hrany dostávají více feromonu a vypařování brání předčasnému uvíznutí ve špatné trase. Zdroj: \href{https://commons.wikimedia.org/wiki/File:Aco\_TSP.svg}{Wikimedia Commons}, autor Nojhan, licence CC BY-SA 3.0.}
\end{figure}
...
\subsection{Praktické nástroje a řešiče}

Optimalizační model je v praxi vhodné oddělit od řešiče. Model popisuje proměnné, účelovou funkci a omezení;
řešič je software, který model numericky vyřeší. Stejný model lze často řešit různými řešiči a porovnávat
rychlost, kvalitu mezí, stabilitu a cenu licence.

Typické nástroje:
\begin{itemize}
    \item \term{tabulkové procesory} -- Excel Solver nebo OpenSolver pro malé výukové a manažerské úlohy;
    \item \term{modelovací jazyky} -- AMPL, GAMS, Pyomo, JuMP nebo PuLP pro zápis modelu oddělený od dat;
    \item \term{otevřené řešiče} -- GLPK, CBC, SCIP, HiGHS;
    \item \term{komerční řešiče} -- Gurobi, CPLEX, FICO Xpress, MOSEK;
    \item \term{aplikační knihovny} -- Google OR-Tools pro LP, MIP, CP-SAT, síťové toky, přiřazování a routing;
    \item \term{vědecké knihovny} -- SciPy pro spojitou optimalizaci a lineární programování přes HiGHS.
\end{itemize}

Při výběru řešiče záleží na typu úlohy:
\begin{itemize}
    \item čisté LP řeší simplexové metody nebo metody vnitřních bodů;
    \item MILP se obvykle řeší branch-and-cut;
    \item úlohy s mnoha logickými podmínkami mohou dobře sedět na CP-SAT;
    \item routingové úlohy se často řeší specializovanými VRP heuristikami;
    \item nelineární úlohy vyžadují jiné metody než LP/MILP.
\end{itemize}

U zkoušky je důležité znát princip modelování. V praxi je stejně důležité umět interpretovat stav řešení:
\begin{itemize}
    \item \term{optimal} -- řešič prokázal optimálnost;
    \item \term{feasible} -- našel přípustné řešení, ale nemusel prokázat optimum;
    \item \term{infeasible} -- omezení si odporují a neexistuje přípustné řešení;
    \item \term{unbounded} -- účelovou funkci lze zlepšovat bez omezení;
    \item \term{gap} -- rozdíl mezi nejlepší známou mezí a nejlepším přípustným řešením.
\end{itemize}
...
\begin{itemize}
    ...
    \item V praxi se používají řešiče a modelovací nástroje; kromě optimální hodnoty je nutné sledovat
    přípustnost, gap, čas výpočtu a interpretovatelnost řešení.
\end{itemize}

\subsection*{Zdroje}
\begin{itemize}
    \item NEOS Guide:
    \href{https://neos-guide.org/guide/types/lp/}{Linear Programming}.
    \item NEOS Guide:
    \href{https://neos-guide.org/guide/types/integer/}{Integer Programming}.
    \item Google OR-Tools:
    \href{https://developers.google.com/optimization/lp}{Linear Optimization}.
    \item Google OR-Tools:
    \href{https://developers.google.com/optimization/mip}{Integer Optimization}.
    \item Google OR-Tools:
    \href{https://developers.google.com/optimization/routing/tsp}{Traveling Salesperson Problem}.
    \item COIN-OR CBC:
    \href{https://www.coin-or.org/Cbc/ch01.html}{Branch-and-Cut Overview}.
    \item SCIP Optimization Suite:
    \href{https://scipopt.org/}{SCIP}.
    \item SciPy:
    \href{https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.linprog.html}{linprog documentation}.
    \item Gurobi:
    \href{https://docs.gurobi.com/projects/optimizer/en/current/index.html}{Optimizer Reference Manual}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:Linear\_Programming\_Feasible\_Region.svg}{Linear Programming Feasible Region}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:Branch\_and\_Bound.png}{Branch and Bound}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:GLPK\_solution\_of\_a\_travelling\_salesman\_problem.svg}{GLPK solution of a travelling salesman problem}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:Aco\_TSP.svg}{Aco TSP}.
\end{itemize}
