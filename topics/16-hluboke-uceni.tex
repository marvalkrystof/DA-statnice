...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.72\linewidth]{assets/images/hluboke-uceni/multilayer-neural-network.png}
  \caption[Vícevrstvá neuronová síť]{Vícevrstvá dopředná neuronová síť. Popis obrázku: vstupní hodnoty
  postupně procházejí přes více skrytých vrstev do výstupní vrstvy. Každá vrstva vytváří novou reprezentaci
  dat a skládáním těchto reprezentací vzniká schopnost modelovat složité nelineární vztahy. Zdroj obrázku:
  Offnfopt, \href{https://commons.wikimedia.org/wiki/File:Multi-Layer\_Neural\_Network-Vector.svg}{Wikimedia
  Commons}, licence CC0 1.0 / public domain.}
  \label{fig:multilayer-neural-network}
\end{figure}
...
\paragraph{Induktivní předpoklady architektur.}
Různé architektury nejsou zaměnitelné. MLP předpokládá obecný vektor příznaků, CNN využívá lokální
prostorovou strukturu a sdílení filtrů, RNN/LSTM pracují se sekvenčním pořadím a transformer se učí vztahy
mezi prvky sekvence pomocí attention. Správná architektura tedy vnáší do modelu předpoklad, který odpovídá
typu dat.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\linewidth]{assets/images/hluboke-uceni/cnn-convolutional-layers.png}
  \caption[Konvoluční vrstvy v CNN]{Konvoluční vrstvy v CNN. Popis obrázku: malý filtr se aplikuje lokálně
  na vstupní mapu a vytváří feature mapu. Stejné váhy filtru se sdílejí napříč pozicemi, takže počet parametrů
  neroste s každým pixelem samostatně a síť využívá prostorovou strukturu obrazu. Zdroj obrázku: Renanar2,
  \href{https://commons.wikimedia.org/wiki/File:CNN\_Convolutional\_Layers.svg}{Wikimedia Commons}, licence
  CC BY-SA 4.0.}
  \label{fig:cnn-convolutional-layers}
\end{figure}
...
Augmentace se typicky aplikuje jen na trénovací data. Validační a testovací množina mají zůstat stabilní,
aby měřily výkon modelu na realistických, nikoli náhodně změněných datech.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.86\linewidth]{assets/images/hluboke-uceni/lstm-cell.png}
  \caption[LSTM buňka]{Vnitřní struktura LSTM buňky. Popis obrázku: brány řízené sigmoidou rozhodují, kolik
  informace se zapomene, kolik se přidá do paměťového stavu a jaká část paměti se použije pro nový skrytý
  stav. Právě paměťový stav pomáhá zachytit delší závislosti v sekvenci. Zdroj obrázku: Guillaume Chevalier,
  \href{https://commons.wikimedia.org/wiki/File:The\_LSTM\_cell.png}{Wikimedia Commons}, licence CC BY 4.0.}
  \label{fig:lstm-cell}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.64\linewidth]{assets/images/hluboke-uceni/transformer-architecture.png}
  \caption[Architektura transformeru]{Původní encoder-decoder transformer. Popis obrázku: encoder převádí
  vstupní sekvenci na kontextové reprezentace, decoder generuje výstup autoregresivně. Obě části používají
  attention, reziduální spojení, normalizaci a feed-forward vrstvy; decoder navíc používá maskování budoucích
  pozic. Zdroj obrázku: Yuening Jia,
  \href{https://commons.wikimedia.org/wiki/File:The-Transformer-model-architecture.png}{Wikimedia Commons},
...
Prakticky se často postupuje ve dvou krocích: nejprve se zmrazí předtrénovaný backbone a naučí se nová
klasifikační hlava, potom se případně odemrazí část vyšších vrstev a celý model se krátce doladí s menším
learning rate. Tím se sníží riziko, že se užitečné předtrénované reprezentace rychle zničí.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.74\linewidth]{assets/images/hluboke-uceni/autoencoder-structure.png}
  \caption[Autoencoder]{Schéma autoencoderu. Popis obrázku: encoder převádí vstup do kompaktního latentního
  kódu a decoder z něj rekonstruuje původní vstup. Pokud je bottleneck dost úzký nebo je model regularizovaný,
  síť se musí naučit užitečnou reprezentaci dat. Zdroj obrázku: Chervinskii,
  \href{https://commons.wikimedia.org/wiki/File:Autoencoder\_structure.png}{Wikimedia Commons}, licence
  CC BY-SA 4.0.}
...
\paragraph{Unsupervised a self-supervised použití.}
U obrazových dat lze použít i metody bez ručních tříd. Častý postup je nejprve převést obrázek na
embedding pomocí CNN nebo vision transformeru a teprve nad embeddingy dělat shlukování, vyhledávání
podobných obrázků nebo detekci anomálií. Další možnosti jsou PCA nad příznaky, autoenkodéry pro redukci
dimenze a rekonstrukční chybu, self-supervised učení reprezentací a segmentace obrazu bez detailních
popisků. U zkoušky je důležité říct, že přímo shlukovat syrové pixely bývá slabé; lepší je pracovat
s vhodnou reprezentací obrazu.
...
\subsubsection{Paralelizace ML a DL}
Klasické ML se často paralelizuje přes nezávislé části práce: trénování stromů v random forestu,
cross-validation foldy, grid search, bootstrap vzorky nebo predikce pro různé dávky dat. U hlubokého
učení je hlavní výhoda v tom, že výpočet tvoří velké maticové a tensorové operace, které dobře běží
na GPU/TPU.

U velkých neuronových sítí se používá:
\begin{itemize}
    \item \term{datová paralelizace} -- kopie modelu běží na více zařízeních a každá zpracuje jiný
    mini-batch;
    \item \term{modelová paralelizace} -- různé části jednoho velkého modelu jsou na různých zařízeních;
    \item \term{pipeline paralelizace} -- vrstvy modelu tvoří výpočetní pipeline.
\end{itemize}

Limitem paralelizace není jen počet procesorů, ale i paměť, přenos gradientů, synchronizace a režie
komunikace.

\subsubsection{Kvantové ML vs. klasické deep learning}
Klasické hluboké učení používá klasické výpočty nad tensory a optimalizuje parametry gradientními
metodami. \term{Kvantové strojové učení} využívá kvantové stavy, superpozici, interference a kvantové
obvody; často se uvažují hybridní modely, kde část výpočtu běží na kvantovém zařízení a část na klasickém
počítači.

Pro zkoušku je bezpečná formulace: kvantové ML není jen \uv{rychlejší deep learning}. Může být užitečné
pro některé speciální výpočty, optimalizaci nebo práci s kvantovými daty, ale praktické použití je zatím
omezené šumem, malým počtem qubitů, obtížnou škálovatelností a nejasnou výhodou pro běžné tabulkové,
textové nebo obrazové úlohy.
...
\begin{itemize}
    ...
    \item PyTorch Tutorials:
    \href{https://docs.pytorch.org/tutorials/recipes/recipes/defining\_a\_neural\_network.html}{Defining a
    Neural Network}.
    \item TensorFlow / Keras Guide:
    \href{https://www.tensorflow.org/guide/keras/transfer\_learning}{Transfer learning and fine-tuning}.
    \item TensorFlow Tutorials:
    \href{https://www.tensorflow.org/tutorials/images/data\_augmentation}{Data augmentation}.
    \item Vaswani et al.:
    \href{https://arxiv.org/abs/1706.03762}{Attention Is All You Need}.
    \item Hugging Face Transformers:
    \href{https://huggingface.co/docs/transformers/v4.16.2/en/model\_summary}{Summary of the models}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:Multi-Layer\_Neural\_Network-Vector.svg}{Multi-Layer Neural
    Network-Vector}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:CNN\_Convolutional\_Layers.svg}{CNN Convolutional Layers}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:The\_LSTM\_cell.png}{The LSTM cell}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:The-Transformer-model-architecture.png}{The Transformer model
    architecture}.
    \item Wikimedia Commons:
    \href{https://commons.wikimedia.org/wiki/File:Autoencoder\_structure.png}{Autoencoder structure}.
\end{itemize}
